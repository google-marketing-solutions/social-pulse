...
from infrastructure.insights.gemini import GeminiInsightsProvider
...
class TestGeminiInsightsProvider(unittest.TestCase):
  ...
  @mock.patch("infrastructure.insights.gemini"
              ".GeminiPromptClient")
  ...
  def test_generate_base_insights_success(self, mock_client_class):
    ...
    Given a report context and a list of top brands
    When generate_base_insights is called
    Then it calls the Gemini client with JSON formatting rules
    and returns the parsed base insights containing top and brand trends.

    Args:
      mock_client_class: The mocked GeminiPromptClient class.
    """
    ...
    expected_response = {
        ...
        "brand_trends": {
            "brand A": {
                "trend_title": "Brand Trend 1",
                "description": "Brand Description 1",
                "justifications": ["Brand Justification 1"]
            }
        }
    }
    ...
    result_json, result_raw = provider.generate_base_insights(
        self.report_context, top_brands=["brand A"])
  ...
  def test_answer_chat_query_success(self, mock_client_class):
    ...
    result = provider.answer_chat_query(self.report_context, chat_history,
                                        "Current query")
