   ...
   report.
    ...
    ```shell
      ...
      curl -X POST 'https://sp-reporting-api-[Your GCP NUMBER].[Project Location].run.app/api/report' \
        -H "Authorization: Bearer $(gcloud auth print-identity-token)" \
        -H 'Content-Type: application/json' \
        -d '{ "sources": [ "YOUTUBE_VIDEO" ], "data_output": "SENTIMENT_SCORE", "topic": "HubSpot Smart CRM", "start_time": "2024-12-01T00:00:00Z", "end_time": "2025-12-01T23:59:59Z", "include_justifications": true }'
    ```
